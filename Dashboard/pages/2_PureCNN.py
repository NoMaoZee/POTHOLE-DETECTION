"""
PureCNN Model Page - Blue + Black Theme
Custom CNN Architecture - 99.49% Accuracy
"""
...
from utils import (
    get_base_css, get_purecnn_theme,
    load_purecnn_model, get_model_info, CLASS_NAMES,
    predict_with_gradcam, compute_image_stats, generate_interpretation
)

# Page config
st.set_page_config(
    page_title="PureCNN - Pothole Detection",
    page_icon="🔵",
    layout="wide"
)
...
st.markdown(get_purecnn_theme(), unsafe_allow_html=True)

# Paths
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
PROJECT_ROOT = os.path.dirname(BASE_DIR)
PURECNN_DIR = os.path.join(PROJECT_ROOT, "PureCNN")

# Title
st.markdown("""
<div style="text-align: center; padding: 1.5rem 0;">
    <h1><i class="fa-solid fa-brain"></i> Pothole Detection AI - PureCNN</h1>
    <p style="font-size: 20px; background: linear-gradient(90deg, #1976D2 0%, #42A5F5 100%); 
       -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-weight: 700;">
        Custom CNN Architecture | Champion Model 🥇
    </p>
</div>
""", unsafe_allow_html=True)

# Load model
model = load_purecnn_model()
model_info = get_model_info("PureCNN")
...
with tab1:
    ...
    with col1:
        st.metric("Accuracy", "99.49%", "Best")
    with col2:
        st.metric("Precision", "99.49%")
    with col3:
        st.metric("Recall", "99.49%")
    with col4:
        st.metric("F1-Score", "99.49%")
    ...
    with col1:
        metrics_img = os.path.join(PURECNN_DIR, "Training", "training_metrics.png")
    ...
    with col2:
        cm_img = os.path.join(PURECNN_DIR, "Evaluation", "confusion_matrix.png")
    ...
    with col1:
        stacked_bar = os.path.join(PURECNN_DIR, "EDA", "stacked_bar.png")
        if os.path.exists(stacked_bar):
            st.image(stacked_bar, caption="Dataset Distribution", use_container_width=True)
    
    with col2:
        sample_pothole = os.path.join(PURECNN_DIR, "EDA", "sample_pothole.png")
        if os.path.exists(sample_pothole):
            st.image(sample_pothole, caption="Sample Pothole Images", use_container_width=True)
    
    # XAI Examples
    st.markdown("---")
    st.markdown("### <i class='fa-solid fa-eye'></i> XAI - Grad-CAM Examples", unsafe_allow_html=True)
    
    gradcam_samples = os.path.join(PURECNN_DIR, "XAI", "gradcam_samples.png")
    if os.path.exists(gradcam_samples):
        st.image(gradcam_samples, caption="Grad-CAM Visualizations", use_container_width=True)
    else:
        st.info("Grad-CAM samples will be generated during inference")
...
with tab2:
    ...
    if uploaded_file is not None:
        ...
        with st.spinner("Processing image..."):
            ...
            # Result box
            result_color = "#1976D2" if pred_label == "POTHOLE" else "#4CAF50"
            st.markdown(f"""
            ...
            colors = ['#1976D2' if c == pred_label else '#4a4a4a' for c in CLASS_NAMES]
            ...
            interpretation = generate_interpretation(pred_label, pred_conf, "PureCNN")
            ...
            if st.button("🚀 Start Batch Analysis", use_container_width=True):
                ...
                for idx, (img, name) in enumerate(zip(images, image_names)):
                    ...
                    pred_label, pred_conf, preds = predict_image(
                        model, img, CLASS_NAMES, model_type="PureCNN"
                    )
                    results.append({
                        "image_name": name,
                ...
                st.download_button(
                    ...
                    data=csv,
                    file_name="purecnn_batch_results.csv",
                    mime="text/csv",
...
<div style="text-align: center; color: #888; padding: 2rem 0;">
    ...
    <p style="margin: 0.5rem 0 0 0; font-size: 12px; color: #666;">
        <i class="fa-solid fa-brain"></i> PureCNN Model - Custom CNN Architecture
    </p>
</div>
""", unsafe_allow_html=True)